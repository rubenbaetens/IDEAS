...
protected
  ...
  algorithm
    rat := V_flow*IDEAS.Utilities.Math.Functions.inverseXRegularized(r_N,1e-10);
    i :=1;
    ...
    dp:=IDEAS.Utilities.Math.Functions.spliceFunction(pos=sign(r_N)*r_N^2,neg=1e-4*r_N,x=r_N-1e-4,deltax=1e-4)*IDEAS.Utilities.Math.Functions.cubicHermiteLinearExtrapolation(
                x=rat,
                x1=per.V_flow[i],
                x2=per.V_flow[i + 1],
                y1=per.dp[i],
